...
💡    Always format currency using Indian Rupees (₹), NOT dollars. Never use the "$" symbol.
        ...
        7. filterExpenses({ category }): Show all expenses for a given category like 'Food' or 'Transport'.
    ...
    try {
      ...
      switch (name) {
        ...
        case "listExpenses":
          ...
          break;
        case "filterExpenses":
          result = await filterExpenses(args);
          break;
        default:
...
async function filterExpenses({ category }: { category: string }) {
  const { data: expenses, error } = await supabase
    .from("expenses")
    .select("*")
